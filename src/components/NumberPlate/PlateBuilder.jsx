//previous code without api integration
...
// Custom hook for fetching and managing configuration data
const useConfigurations = () => {
    ...
    // Transform database data to match PlateJson format
    const transformConfigurations = (configurations) => {
        const transformed = {};
        
        // Transform each configuration type
        Object.entries(configurations).forEach(([type, configs]) => {
            switch (type) {
                case 'plateStyle':
                    transformed.plateStyles = configs.map(config => ({
                        key: config.key,
                        label: config.label,
                        price: config.price,
                ...
                case 'thickness':
                    transformed.thicknessOptions = configs.map(config => ({
                        key: config.key,
                        label: config.label,
                        price: config.price,
                        value: config.value
                ...
                case 'color':
                    transformed.colorOptions = configs.map(config => ({
                        color: config.color,
                        name: config.name,
                        price: config.price
                    }));
                ...
                case 'shadow':
                    transformed.shadowOptions = configs.map(config => ({
                        key: config.key,
                        name: config.name,
                        price: config.price
                    }));
                ...
                case 'border':
                    transformed.borderOptions = configs.map(config => ({
                        key: config.key,
                        name: config.name,
                        price: config.price,
                ...
                case 'size':
                    transformed.sizeOptions = configs.map(config => ({
                        key: config.key,
                        label: config.label,
                        price: config.price,
                ...
                case 'finish':
                    transformed.finishOptions = configs.map(config => ({
                        key: config.key,
                        label: config.label,
                        price: config.price,
                        description: config.description
                ...
                case 'country':
                    transformed.countryOptions = configs.map(config => ({
                        key: config.key,
                        name: config.name
                    }));
                    break;
                ...
                case 'flag':
                    // Group flags by parent country
                    const flagsByCountry = {};
                    configs.forEach(config => {
                        if (!flagsByCountry[config.parentCountry]) {
                            flagsByCountry[config.parentCountry] = [];
                        }
                        flagsByCountry[config.parentCountry].push({
                            key: config.key,
                            name: config.name,
                            text: config.text,
                            price: config.price,
                    ...
                    transformed.flagOptions = flagsByCountry;
                    break;
            }
        });
    ...
    const fetchConfigurations = async () => {
        ...
        try {
            ...
            const [
                ...
                apiCall('/config/plateStyle'),
                apiCall('/config/thickness'),
                apiCall('/config/color'),
                apiCall('/config/shadow'),
                apiCall('/config/border'),
                apiCall('/config/size'),
                apiCall('/config/finish'),
                apiCall('/config/country'),
                apiCall('/config/flag')
            ]);

            const rawConfigurations = {
                plateStyle: plateStyles,
            ...
            const transformedData = transformConfigurations(rawConfigurations);
            ...
            try {
                const module = await import('../../config/PlateJson');
                setConfigurations({
                    plateStyles: module.plateStyles,
                    thicknessOptions: module.thicknessOptions,
                    colorOptions: module.colorOptions,
                    shadowOptions: module.shadowOptions,
                    borderOptions: module.borderOptions,
                    sizeOptions: module.sizeOptions,
                    finishOptions: module.finishOptions,
                    countryOptions: module.countryOptions,
                    flagOptions: module.flagOptions
                });
