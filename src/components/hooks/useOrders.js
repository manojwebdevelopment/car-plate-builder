...
export const useOrders = () => {
  ...
  const fetchOrders = useCallback(
    async (customFilters = null, customPagination = null) => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/admin/orders?${queryParams.toString()}`,
          {
  ...
  const fetchOrderDetails = useCallback(async (orderId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/order-details/${orderId}`,
        {
          headers: getAuthHeaders(),
        }
      );
      ...
      return {
        success: false,
        error: "Network error: Unable to fetch order details",
      };
    } finally {
  ...
  const updateOrderStatus = useCallback(async (orderId, newStatus) => {
    try {
      const response = await fetch(
        `http://localhost:5000/admin/orders/${orderId}/quick-status`,
        {
          method: "PATCH",
          headers: getAuthHeaders(),
  ...
  const exportOrders = useCallback(
    async (exportType = "current", customQueryParams = null) => {
      setExporting(true);
      try {
        let queryParams;

        if (customQueryParams) {
          queryParams = new URLSearchParams(customQueryParams);
        } else {
          ...
          // Only add pagination for 'current' export
          if (exportType === "current") {
            queryParams.append("page", pagination.currentPage);
            queryParams.append("limit", pagination.itemsPerPage);
          }

          // Add filters for 'current' and 'filtered' exports
        ...
        console.log(
          `Starting export: ${exportType} with params:`,
          queryParams.toString()
        );

        const response = await fetch(
          `http://localhost:5000/admin/orders/export?${queryParams.toString()}`,
          {
            headers: {
        ...
        if (response.ok) {
          ...
          // Success messages based on export type
          const messages = {
            current: `Current view exported successfully! (${filename})`,
            filtered: `All filtered orders exported successfully! (${filename})`,
            all: `All orders exported successfully! (${filename})`,
          };

          console.log(`✅ Export completed: ${filename}`);
          return {
            success: true,
            message: messages[exportType] || "Orders exported successfully!",
            filename,
          };
        } else {
          ...
          return {
            success: false,
            error: errorResult.error || "Export failed",
          };
        }
      } catch (error) {
        ...
        return {
          success: false,
          error: "Network error: Unable to export orders",
        };
      } finally {
        setExporting(false);
      }
